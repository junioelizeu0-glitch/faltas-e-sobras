          ...
          <div onClick={(e) => e.stopPropagation()} className="bg-white rounded-xl shadow-2xl w-full max-w-lg">
            ...
            <div className="p-6 space-y-5">
              <div className="grid grid-cols-2 gap-5">
                <label className="block"><span className="text-xs font-semibold text-slate-600">Referência *</span>
                  <input autoFocus value={editing.referencia || ""} onChange={(e) => setEditing({ ...editing, referencia: e.target.value })} className="mt-1.5 w-full text-sm rounded-md border border-slate-300 px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-blue-500"/></label>
                <label className="block"><span className="text-xs font-semibold text-slate-600">Cor</span>
                  <input value={editing.cor || ""} onChange={(e) => setEditing({ ...editing, cor: e.target.value })} className="mt-1.5 w-full text-sm rounded-md border border-slate-300 px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-blue-500"/></label>
              </div>
              <label className="block"><span className="text-xs font-semibold text-slate-600">Descrição</span>
                <input value={editing.descricao || ""} onChange={(e) => setEditing({ ...editing, descricao: e.target.value })} className="mt-1.5 w-full text-sm rounded-md border border-slate-300 px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-blue-500"/></label>
              <label className="block"><span className="text-xs font-semibold text-slate-600">Fornecedor</span>
                <input value={editing.nome_parceiro || ""} onChange={(e) => setEditing({ ...editing, nome_parceiro: e.target.value })} className="mt-1.5 w-full text-sm rounded-md border border-slate-300 px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-blue-500"/></label>
            </div>
            <div className="flex justify-end gap-2 px-6 py-4 border-t bg-slate-50">
              ...
              <button onClick={save} className="inline-flex items-center gap-1.5 px-4 py-2 text-sm text-white bg-blue-600 hover:bg-blue-700 rounded-md"><Save className="w-4 h-4"/>Salvar</button>
            </div>
          </div>
        </div>
      )}
