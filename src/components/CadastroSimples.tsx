      ...
      <div className="w-full">
        ...
        {loading ? (
          ...
          <div className="grid gap-2 [grid-template-columns:repeat(auto-fill,minmax(220px,1fr))]">
            {filtered.map((r) => (
              <div key={r.id} className="group relative bg-white border border-slate-200 rounded-lg px-3 py-2.5 hover:border-blue-300 hover:shadow-sm transition-all">
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0 flex-1">
                    <div className="text-sm font-semibold text-slate-800 truncate" title={r.nome}>{r.nome}</div>
                    {extraFields.map((f) => (
                      r[f.key] ? (
                        <div key={f.key} className="text-[11px] text-slate-500 mt-0.5">
                          <span className="font-semibold">{f.label}:</span> {String(r[f.key])}
                        </div>
                      ) : null
                    ))}
                  </div>
                  <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => setEditing(r)} title="Editar" className="p-1.5 text-blue-600 hover:bg-blue-50 rounded"><Pencil className="w-3.5 h-3.5"/></button>
                    <button onClick={() => remove(r)} title="Excluir" className="p-1.5 text-red-500 hover:bg-red-50 rounded"><Trash2 className="w-3.5 h-3.5"/></button>
                  </div>
                </div>
              </div>
            ))}
